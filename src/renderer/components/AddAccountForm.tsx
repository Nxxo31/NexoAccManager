...
export default function AddAccountForm({ onSuccess }: AddAccountFormProps) {
  ...
  return (
    <div className="bg-dark rounded-lg p-4">
      ...
      {limitReached && (
        <div className="p-3 bg-error/30 border border-error/50 rounded mb-4 text-sm text-error">
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        <div>
          ...
          <textarea
            ...
            className="w-full bg-dark border border rounded-md px-3 py-2 text-sm text-primary placeholder-gray-600 resize-none focus:outline-none focus:border-primary transition-colors"
            rows={4}
            disabled={loading}
          />
        </div>
        ...
        <div>
          ...
          <input
            ...
            className="w-full bg-dark border border rounded-md px-3 py-2 text-sm text-primary focus:outline-none focus:border-primary transition-colors"
            disabled={loading}
          />
        </div>

        {error && (
          <div className="p-2 bg-error/30 border border-error/50 rounded text-xs text-error">
            {error}
          </div>
        )}
        ...
        <button
          ...
          className="w-full py-2 bg-primary text-primary text-sm font-medium rounded-md hover:bg-[color:var(--primary)_0.9] transition-all disabled:opacity-50 disabled:cursor-not-allowed"
        >
