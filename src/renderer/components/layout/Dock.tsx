  ...
  return (
    <>
      ...
      <div className="flex-shrink-0 flex items-center gap-1.5 px-4 py-2 border-t border-border bg-bg-card">
        ...
        <button
          onClick={() => selectedAccount && setSelectedAccount(null)}
        ...
        <div className="relative">
          {dropdownOpen && (
            <div className="absolute right-0 bottom-full mb-2 w-56 rounded-md bg-bg-card border border-border shadow-lg z-50">
              <div className="py-1">
                <button onClick={() => { if (selectedAccount) { onCopyPlaceId(placeId); setDropdownOpen(false); } }} className="w-full flex items-center gap-2 px-3 py-2 text-xs text-muted-foreground hover:text-white hover:bg-bg-surface">
                  <Copy className="h-3.5 w-3.5" aria-hidden="true" /> Copiar Place ID
                </button>
                <button onClick={() => { if (selectedAccount) { onCopyRbxlLink(selectedAccount); setDropdownOpen(false); } }} className="w-full flex items-center gap-2 px-3 py-2 text-xs text-muted-foreground hover:text-white hover:bg-bg-surface">
                  <Copy className="h-3.5 w-3.5" aria-hidden="true" /> Copiar rbx-player link
                ...
                <button onClick={() => { if (selectedAccount) { onToggleAutoRelaunch(selectedAccount); setDropdownOpen(false); } }} className="w-full flex items-center gap-2 px-3 py-2 text-xs text-muted-foreground hover:text-white hover:bg-bg-surface">
                ...
                <button onClick={() => { if (selectedAccount) { onToggleConnectionWatcher(selectedAccount); setDropdownOpen(false); } }} className="w-full flex items-center gap-2 px-3 py-2 text-xs text-muted-foreground hover:text-white hover:bg-bg-surface">
